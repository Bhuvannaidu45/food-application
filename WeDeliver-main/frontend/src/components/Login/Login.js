...
const Login = () => {
  ...
  const registerUser = async (currentUser) => {
    try {
      setLoading(true);
      const response = await axios.post(`https://foodie-bhuvan-projects-abd5e9c0.vercel.app/api/register`, currentUser);
  ...
  const loginUser = async (currentUser) => {
    try {
      setLoading(true);
      const response = await axios.post(`https://-foodie-bhuvan-projects-abd5e9c0.vercel.app/api/login`, currentUser);
      const { user, token } = response.data;
      addUserToLocalStorage({ user, token });
      if (user) {
        setLoading(false);
    ...
    <div className="login">
      <div className="login-container" ref={focusLogin}>
        ...
        <div className="signUpLine">
          {!isMember ? "Already Signed Up?" : "New to foodie?"}{" "}
